...
**User request:** Align Auto, Loriane, Marc always with the Profil retenu block. Today the chips hug their labels on the left while Profil retenu is full width of the sidebar. Stretch the chips so left and right edges match that card, in both 2-chip (no spouse) and 3-chip (couple) layouts.
...
Append to `src/features/profile.test.ts` a new `describe("principal mode chips")`:
